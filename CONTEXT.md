...
- CI 已升级为分层门禁：后端模块单测 + 集成测试、前端 lint/test/build、Maven Checkstyle、`scripts/regression.sh`、`scripts/verify-supplement.sh`、`scripts/user-journey-blackbox.sh` 全部纳入 PR / push 到 `main` 的工作流；本地默认门禁仍保持 `compile + frontend lint/test/build`，黑盒按改动范围补跑；**压测自 2026-08-30 起不再作为门禁（用户决定：功能正常即可），性能结论以既有基线为准**
